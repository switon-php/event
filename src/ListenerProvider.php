...
class ListenerProvider implements ListenerProviderInterface
{
    ...
    /** Registers one listener for one event type; lower priority value runs earlier. */
        ...
        foreach ($rClass->getMethods(ReflectionMethod::IS_PUBLIC) as $rMethod) {
            ...
            if (count($rParameters = $rMethod->getParameters()) !== 1) {
                MisuseException::raise(
                    '{listener}::{method}() marked with #[EventListener] must declare exactly one parameter',
