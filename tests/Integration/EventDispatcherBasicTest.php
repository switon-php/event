 ...
 */
final class TestableEventDispatcher extends EventDispatcher
{
    public function disableListenerProvider(): void
    {
        $this->listenerProvider = new NullListenerProvider();
    }
}
...
class EventDispatcherBasicTest extends TestCase
{
    ...
    /** Test that dispatch() returns the event unchanged when a no-op listener provider is wired. */
    public function testDispatchReturnsEventWhenListenerProviderIsNoOp(): void
    {
        $event = new TestEvent('no provider');
        $dispatcher = $this->container->make(TestableEventDispatcher::class);
        $dispatcher->disableListenerProvider();
